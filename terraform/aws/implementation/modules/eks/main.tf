...
data "external" "chart_versions" {
  ...
  program = ["helm", "search", "repo", "phdi-charts", "-o", "json", "|", "jq", "'reduce .[] as $item ({}; .[$item.name | sub(\"^phdi-charts/\";\"\")] |= if . == null then $item.version else if . < $item.version then $item.version else . end end)'"]
}
